...
void loop()
{
  if (millis() - failsafeTimerLastCheck > FAILSAFE_CHECK_INTERVAL)
  {
    ...
    */
    ...
    Serial.print("HK (23): ");
    Serial.print(digitalRead(HARDWAREKILLSWITCHTRIG));
    Serial.print("  |  SK (22): ");
    Serial.print(digitalRead(SOFTWAREKILLSWITCHTRIG));
    Serial.print("  |  SM (21): ");
    Serial.print(digitalRead(SOFTWAREOPERATINGMODE));
    Serial.print("  |  HM (20): ");
    Serial.print(digitalRead(HARDWAREOPERATINGMODE));
    Serial.print("  |  Mode: ");
    Serial.print(operationMode);
    Serial.print("  |  FailsafeTRIG?: ");
    Serial.print(failsafeTriggered);
    Serial.println();
  }
}
...
{
  if (bytes == 8)
  { // Check if 8 bytes (4 x 16-bit integers) are received
    int PWM_values[4];
